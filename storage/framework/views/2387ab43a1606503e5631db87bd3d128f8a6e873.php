...
<script>
    ...
    $("table").on("change",".productImage",function(){
        loadoverlay($(this).closest("tr").find(".image-preview"));
            const [file] = this.files
            if (file) {
                hideoverlay($(this).closest("tr").find(".image-preview"));
                $(this).closest("tr").find(".image-preview").prop("src",URL.createObjectURL(file));           

            }
    })
    $("table").on("click", ".addRow", function() {
        var tr = $(this).closest("tr")
		tr.after($(this).closest("tr").clone())
        $(this).replaceWith(`<button class="btn btn-sm btn-danger btn-pill removeRow"><i class="fa fa-minus-circle"></i></button>`)
	})
	$("table").on("click", ".removeRow", function() {
        if(confirm("Are you sure you want to delete?"))
		$(this).closest("tr").remove()
	})
    $("table").on("change",".select_all, #select_all", function(e) {
            e.preventDefault()
            $(".item_row").prop("checked", $(this).prop("checked")).trigger("change")
  ...
  const elem = document.createElement('textarea');
   ...
   document.body.removeChild(elem);
    ...
    const getSelectedItems = (table) => {
        return $(table).find(".item_row").filter(function(){
...
<?php /**PATH C:\Users\subra\Documents\projects\kedarCollection\resources\views/adminpanel/script.blade.php ENDPATH**/ ?>