...
@section('content')
<x-modal id="setPriceModal" aria-labelledby="setPriceModal">
	<table>
		<thead>
			<tr>
		...
		<tbody class="setPriceBody">
			...
			<tr class="price_row border-bottom">
			    <td><input type="number" name="quantity[]" class="form-control quantity m-1 mr-4 ml-4" value="{{$qty->quantity}}"></td>
				<td><input type="number" name="price[]" class="form-control price m-1 mr-4 ml-4" value="{{$qty->price}}"></td>
...
<x-modal id="setUploadModal" aria-labelledby="setUploadModal">
	<table>
		<thead>
			<tr>
		...
		@if(isset($product))
			...
			<tr class="price_row p-2">
				<td><input type="file" class="form-control productImage varImage col-md-10 d-none"><input type="hidden" class="image_path" value="{{$image->image}}"><small class="text-primary statusText"></small></td>
				<td class="text-center">
					<div class="p-2 m-1 border"><img src="/{{$image->image}}" width="100px" height="100px" alt="" class="img-fluid image-preview"></div>
				</td>
				<td>
					<label class="switch">
						<input type="checkbox" checked="{{!empty($image->status)}}" data-bs-original-title="" title="" id="" class="product_image_status"><span class="switch-state"></span>
					</label>
				</td>
			...
			<tr class="price_row p-2">
				...
				<td class="text-center">
					<div class="p-2 m-1 border"><img src="{{asset('assets/images/product/1.png')}}" width="100px" height="100px" alt="" class="img-fluid image-preview"></div>
				</td>
				<td>
					<label class="switch">
...
<x-modal id="setVariantModal" aria-labelledby="setVariantModal">
	<table>
		<thead>
			<tr>
		...
		<tbody class="setVariantBody">
		    @if(isset($product) && !empty($product->variants->count()))
			@foreach($product->variants as $variantElem)
			<tr class="price_row price_row p-2">
				<td>
				<select name="variant" id="" class="form-control variant">
					@foreach($variants as $variant)
					<option value="{{@$variant->id}}" @if($variantElem->variant->id == $variant->id) selected @endif>{{$variant->name}}</option>
					@endforeach
				</select>
			    </td>
				<td class="text-center">
					<select name="variant_values[]" id="" class="variant_values" multiple="multiple">
						@foreach($variantElem->variant->productVariantValues->where("product_id",$product->id) as $elem)
						<option value="{{$elem->value->id}}" selected>{{$elem->value->label}}</option>
						@endforeach
					</select>
				</td>
				<td class="text-center"><button class="btn btn-sm btn-danger btn-pill removeRow"><i class="fa fa-minus-circle"></i></button></td>
...
<x-modal id="addVariantModal" aria-labelledby="addVariantModal">
	...
	<table>
		...
		<tbody class="addVariantBody">
			...
			<tr class="price_row border-bottom">
				...
				<td class="text-center">
					<div class="p-2 m-1 border"><img src="{{asset('assets/images/product/1.png')}}" width="70px" height="70px" alt="" class="img-fluid image-preview"></div>
				</td>
				<td class="text-center"><button class="btn btn-sm btn-success addRow btn-pill"><i class="fa fa-plus-circle"></i></button></td>
			</tr>
		</tbody>
	...
	<div class="row">
	    <div class="col-sm-12 mt-1">
			<div class="card">
				...
				<div class="card-body">
				<h4  class="bg-white text-primary p-3  rounded">@if(isset($product))Update @else Add @endif Product</h4>
					<div class="row">
						<div class="col-sm-6 col-md-3">
							<div class="mb-3 mt-1  shadow">
								<img src=" @if(isset($product))	/{{$product->image}} @else {{asset('assets/images/product/1.png')}} @endif" class="img-thumbnail" id="img_prv">
							</div>
						</div>
						<div class="col-md-9">
								...
								<div class="row">
									<div class="col-md-12">
										<div class="mb-3">
											...
											<input class="form-control" type="text" value="{{@$product->name}}" placeholder="Title/name" data-bs-original-title="" id="name" required>
										</div>
									</div>
									...
									<div class="col-sm-6 col-md-6">
										<div class="mb-3">
											...
											<select class='form-control' required id="category">
												...
												<option value="{{$category->id}}" @if(isset($product) && $category->id == $product->category)selected @endif>{{$category->name}}</option>
												@endforeach
											</select>
										</div>
									</div>
									<div class="col-sm-6 col-md-6">
										<div class="mb-3">
											...
											<select class='form-control' id="sub_category">
												...
												<option value="{{$category}}"  @if(isset($product) && $category == $product->sub_category)selected @endif>{{$category}}</option>
												@endforeach
											</select>
										</div>
									</div>
									<div class="col-sm-6 col-md-12">
										<div class="mb-3">
											...
											<select class="form-control" multiple="multiple" id="tags_json">
												@if(isset($product))
												@foreach(json_decode($product->tags_json) as $tag)
						...
						<form class="row" id="product_form_2">
							...
							<div class="col-sm-6 col-md-2">
								<div class="mb-3">
									...
									<input class="form-control" type="number" placeholder="Current Price " value="{{@$product->pre_price}}" data-bs-original-title="" title="" required id="pre_price">
								</div>
							</div>
							<div class="col-sm-6 col-md-2">
								<div class="mb-3">
									<label class="form-label">Offer Price <i class="fa fa-inr"></i></label>
									<input class="form-control" type="number" placeholder="Offer Price " value="{{@$product->price}}" data-bs-original-title="" title="" required id="price">
								</div>
							</div>
							<div class="col-sm-6 col-md-3">
								<div class="mb-3">
									<label class="form-label">Stock <small>(in units)*</small></label>
									<input class="form-control" type="number" placeholder="Stock in units" data-bs-original-title="" title="" value="{{@$product->stock}}" min="0" id="stock">
								</div>
							</div>
							<div class="col-sm-6 col-md-2">
								<div class="mb-3">
									...
									<label class="switch">
										<input type="checkbox" checked="{{@!empty($product->status)}}" data-bs-original-title="" title="" id="status"><span class="switch-state bg-success"></span>
									</label>
								</div>
							</div>
							...
							<div class="row">
								...
								<div class="col-md-6">
									<button class="btn btn-block btn-primary " id="submit"><i class="fa fa-arrow-up">  @if(isset($product)) Update @else Create @endif product</i>  </button>
		...
		if ($("#product_form").valid() && $("#product_form_2").valid()) {
			...
			await	$.ajax(settings).done(async function(response) {
				...
				hideoverlay($("#product_form"));
					...
					var settings = {
						"url":  "@if(isset($product)) /api/update-product/{{@$product->id}}  @else /api/create-product @endif",
						"method": "POST",
						"timeout": 0,
						"processData": false,
						"mimeType": "multipart/form-data",
