...
<div class="col-md-12 pl-5">
    <ul class="mb-3" id="order_details">
        ...
        <li class="p-1 border-bottom">
           {{$item->quantity}} x {{$item->product->name ?? @$item->item->name  }} <strong class="pull-right mr-5">₹ {{$item->subtotal}}</strong>
        </li>
        @endforeach
    </ul>
    <div class="container">
 ...
 minutes > 1 ? $("#readyMessage").html(`We still have ${minutes} minutes left.`) : ""
  ...
  if (distance < 0) {
    ...
    $("#time_left").html(`Please ready order ASAP !`)
        ...
        @elseif($order->current_status == 'order_ready')
...
<div class="container">
    ...
    <div class="col-9 p-2">
            <div class="p-2">
                <ul class="list m-1 p-1">
                    @foreach($order->orderDetails as $item)
                    <li>{{$item->quantity}} x {{$item->product->name ?? @$item->item->name}}</li>
                    @endforeach
                </ul>
            </div>
        </div>
    ...
    <div class="col-md-7">
          ...
          <div class="col-md-5">
          <button class="btn  readyOrder btn-success text-right" data-order_id="{{$order->order_id}}" onclick="packOrder(event,this)"><i class="fa fa-check-circle"></i> Order packed & handed over to Courier service</button>
          </div> 
    </div>
    </div>
</div>
